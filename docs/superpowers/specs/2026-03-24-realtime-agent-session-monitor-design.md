...
2. **后端立即推送**：从缓存获取最新状态并推送（无需等待下次轮询）
3. **后端定时轮询**：每 15 秒调用 Gateway API
4. **状态变化时推送**：对比上次推送的数据，只推送变化的部分
5. **异常标记**：Session 状态为 error/failed 时标记

### 5.1 SSE 心跳机制

- **Keepalive**：每 12 秒发送一次 SSE comment（`:` 开头），保持连接活跃
- **Spring SseEmitter 超时**：设置为 30 秒无活动自动断开
- **前端检测**：若 20 秒内未收到任何消息，判定连接断开，触发重连

### 5.2 断线重连策略

- **重试间隔**：指数退避，1s → 2s → 4s → 8s → 16s（最大）
- **最大重试次数**：5 次
- **重试失败处理**：显示"连接中断"提示，用户可手动点击重连
- **Gateway 不可达处理**：
  - 轮询失败时，将该实例标记为 `offline` 状态
  - 推送 `{instance: {status: "offline"}}` 通知前端
  - 继续定时重试，恢复后自动推送 `online` 状态

### 5.3 活动列表边界

- 每实例保留最近 **20 条**活动记录
- 超出时丢弃最旧的记录
- 活动类型：`session_created`, `session_ended`, `session_error`, `agent_registered`, `agent_unregistered`

### 5.4 数据变化检测

- 对比字段：`sessions[].status`, `agents[].status`, `agents[].sessionCount`
- 若任一字段发生变化，即触发推送
- 仅推送变化的数据（partial update）或完整状态（full update）均可
...
1. **Gateway API 规格未知**：当前基于现有 `GatewayService.invokeTool()` 模式调用 `/tools/invoke`，实际响应格式需在实现时验证
2. **认证未处理**：SSE 端点暂未考虑认证，后续需结合项目整体认证方案
3. **Phase 3 含糊**：调试优化阶段的具体 deliverables 需在 Phase 2 完成后补充
